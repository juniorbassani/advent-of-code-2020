...
fn get_acc(instructions: Vec<String>) -> isize {
    let mut acc = 0;
    let mut i = 0;
    ...
    while let Some((instruction, visited @ false)) = instructions.get_mut(i) {
        *visited = true;
...
pub fn part2() -> isize {
    ...
    let res = helper(&mut instructions, 0);
